...
NSString *cascade_name = [[NSBundle mainBundle] pathForResource:@"haarcascade_frontalface_alt2"
                                                            ofType:@"xml"];
...
- (void)didCaptureIplImage:(IplImage *)iplImage
{
    ...
    //Detect faces in the images
     ...
     IplImage* temp = cvCreateImage( cvSize(iplImage->width/scale,iplImage->height/scale), 8, 3 );
     
     // Create two points to represent the face locations
    CvPoint pt1, pt2;
    
    
    const char* cascade_name_modified = [cascade_name UTF8String];
    size_t len = strlen(cascade_name_modified) + 1;
    
    char mac [len];
    memcpy(mac, cascade_name_modified, len);
    
    
     // Load the HaarClassifierCascade
     cascade = (CvHaarClassifierCascade*)cvLoad(cascade_name_modified, 0, 0, 0 );
     ...
     CvSeq* faces = cvHaarDetectObjects( iplImage, cascade, storage,
     ...
     cvRectangle( iplImage, pt1, pt2, CV_RGB(255,0,0), 3, 8, 0 );
     }
     }
     ...
     //If faces are detected then capture the picture
     ...
     if(i > 0)
     {
         ...
         NSLog(@"Face Detected");
         NSLog(@"Detected Faces : %d", i);
     //Convert the iplImage to UIImage and store it into gallery
     ...
     //Convert image to UIImage to store in gallery
     
     //Store UIImage to gallery
    ...
    else
    {
        ...
        for (size_t i = 0; i < circles.size(); i++)
        {
            ...
            {
            [self.Romo3 stopDriving];
            [self.Romo say:@"Hello Bhargava. How are you?"];
            self.Romo.expression=RMCharacterExpressionChuckle;
            self.Romo.emotion=RMCharacterEmotionHappy;
                [self turnCameraOff];
            [mManager stopAccelerometerUpdates];
                ...
                [self.Romo say:@"Hello Ramesh. How are you?"];
                [self.Romo3 stopDriving];
