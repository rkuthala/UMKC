 ...
 * object.
 */
public class CustomAdapter extends ArrayAdapter<DetectedActivity> {
    ...
    protected void updateActivities(ArrayList<DetectedActivity> detectedActivities) {
        ...
        for (int i = 0; i < Utils.MONITORED_ACTIVITIES.length; i++) {
            int confidence = detectedActivitiesMap.containsKey(Utils.MONITORED_ACTIVITIES[i]) ?
                    detectedActivitiesMap.get(Utils.MONITORED_ACTIVITIES[i]) : 0;

            tempList.add(new DetectedActivity(Utils.MONITORED_ACTIVITIES[i],
                    confidence));
        }
